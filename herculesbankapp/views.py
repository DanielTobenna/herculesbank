...
def contact(request):
	if request.method == 'GET':
		...
		if form.is_valid():
			...
			try:
				send_mail(name, "Investor {} has sent a message saying: {}".format(email, message),email, ['customercare@standardtrust.in'])
...
def transactionhistory(request):
	return render(request, 'herculesbankapp/clienttransactionhistorypage.html')
...
def admincreateaccount(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			print(firstName)
			template= render_to_string('herculesbankapp/welcome.html', {'name':firstName})
			email_message= EmailMessage(
